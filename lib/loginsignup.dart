...
class _LoginPageState extends State<LoginPage> {
  ...
  try {
    final res = await http.post(
      Uri.parse("https://api.chandus7.in/api/infumedz/login/"),
      headers: {"Content-Type": "application/json"},
      body: jsonEncode({
        "identifier": _GetUsername.text.trim(),
    ...
    if (res.statusCode == 200) {
      ...
      final res2 = await http.get(
        Uri.parse("https://api.chandus7.in/api/infumedz/user/$userId/"),
      );

      if (res2.statusCode == 200) {
        final data2 = jsonDecode(res2.body);

        await UserSession.saveUsername(data2["name"]);
        await UserSession.saveUseremail(data2["email"]); // ✅ FIXED
        await UserSession.saveUserphonenumber(data2["phone"]);
      }
